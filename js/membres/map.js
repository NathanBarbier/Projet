...
function init()
{
    ...
    $(".task-bubble-input").off('focus').focus(function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for(i = 0; i < l; i++)
                {
                    $("#task-comment-container").prepend("<div class='task-comment-div'><input type='hidden' class='comment-task-id' value='"+comments[i].rowid+"'><textarea class='mt-3 card task-comment px-2 pt-3 text-center' name='' cols='30' rows='3'>"+comments[i].note+"</textarea></div>")
                }
            }
        });
    });
    ...
    $("#add-comment-btn").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#task-comment-container").prepend("<div class='task-comment-div'><input type='hidden' class='comment-task-id' value='"+commentId+"'><textarea class='mt-3 card task-comment px-2 text-center' name='' cols='30' rows='3'></textarea></div>")
    ...
    $(".task-comment").keyup(function() {
        taskNote = $(this).val();
    });

    $("#check-comment-btn").click(function() {
        $.ajax({
