            ...
            case 'loadmore':
                if($offset)
                {
                    try {
                        ...
                        // echo json_encode($th);
                        echo json_encode(false);
                        LogHistory::create($idOrganization, $idUser, "ERROR", 'loadmore', 'associates', '', '', null, $th->getMessage());
                    }
                }
                break;
            case 'search':
                if($query)
                {
                    try {
                        ...
                        // echo json_encode($th);
                        echo json_encode(false);
                        LogHistory::create($idOrganization, $idUser, "ERROR", 'search', 'associates', '', '', $query, $th->getMessage());
                    }
                }
                break;
        }
